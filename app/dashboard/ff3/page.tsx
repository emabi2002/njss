...
import { Plus, Download, Search, Eye, Edit, CheckCircle2, Clock, XCircle, FileText, Loader2, AlertCircle } from "lucide-react"
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {can('ff3.create') && (
          <Link href="/dashboard/ff3/new" className="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors flex items-center gap-2">
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-4">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Search by FF3 number, purpose, or section..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      <div className="bg-white rounded-lg border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 text-slate-500">
            ...
            <Link href="/dashboard/ff3/new" className="mt-2 text-blue-600 hover:text-blue-700 text-sm font-medium">Create your first requisition →</Link>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">FF3 Number</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Date</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Purpose</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Workflow Status</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Budget Control</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Urgency</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3"><Link href={`/dashboard/ff3/${record.ff3_number}`} className="font-medium text-blue-600 hover:text-blue-700">{record.ff3_number}</Link></td>
                    <td className="px-4 py-3 text-sm text-slate-600">{new Date(record.request_date).toLocaleDateString('en-GB')}</td>
                    <td className="px-4 py-3 text-sm text-slate-900 max-w-xs truncate">{record.purpose}</td>
                    <td className="px-4 py-3 text-sm text-slate-600">{record.section?.name || '-'}</td>
                    <td className="px-4 py-3 text-sm text-slate-900 text-right font-medium">K {(record.total_estimated_amount || 0).toLocaleString()}</td>
                    <td className="px-4 py-3"><StatusBadge status={record.status} /></td>
                    <td className="px-4 py-3"><BudgetControlBadge status={record.budget_control_status} shortfall={record.budget_shortfall_amount || 0} source={record.budget_control_source} /></td>
                    <td className="px-4 py-3"><UrgencyBadge urgency={record.urgency_level || 'MEDIUM'} /></td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link href={`/dashboard/ff3/${record.ff3_number}`} className="p-2 hover:bg-slate-100 rounded" title="View"><Eye className="h-4 w-4 text-slate-600" /></Link>
                        {record.status === "DRAFT" && <Link href={`/dashboard/ff3/${record.ff3_number}/edit`} className="p-2 hover:bg-slate-100 rounded" title="Edit"><Edit className="h-4 w-4 text-slate-600" /></Link>}
                      </div>
                    </td>
                  </tr>
                ))}
...
function StatCard({ label, value }: { label: string; value: number }) {
  return <div className="bg-white rounded-lg border border-slate-200 p-4"><p className="text-xs font-medium text-slate-600 uppercase mb-1">{label}</p><p className="text-2xl font-bold text-slate-900">{value}</p></div>
}

function BudgetControlBadge({ status, shortfall, source }: { status: string | null; shortfall: number; source: string | null }) {
  if (status === 'INSUFFICIENT_BUDGET_BLOCKED') {
    return (
      <div className="space-y-1">
        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700"><AlertCircle className="h-3.5 w-3.5" />INSUFFICIENT BUDGET</span>
        <p className="text-xs text-red-600">COMMITMENT BLOCKED · Shortfall K {shortfall.toLocaleString()}</p>
      </div>
    )
  }
  if (status === 'SUFFICIENT') return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700"><CheckCircle2 className="h-3.5 w-3.5" />Budget OK</span>
  if (status === 'MAPPING_REQUIRED') return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-700"><AlertCircle className="h-3.5 w-3.5" />Mapping Required</span>
  return <span className="text-xs text-slate-500">{source ? `${source} · ` : ''}Not assessed</span>
}
...
function StatusBadge({ status }: { status: FF3Status }) {
  ...
  return <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${config.classes}`}><Icon className="h-3.5 w-3.5" />{config.label}</span>
}
...
function UrgencyBadge({ urgency }: { urgency: string }) {
  ...
  return <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium ${urgencyConfig[urgency] || urgencyConfig.MEDIUM}`}>{urgency}</span>
}
