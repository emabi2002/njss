...
const TABS: TabConfig[] = [
  ...
  { key: "sections", table: "sections", label: "Organisational Sections", select: "id, code, name, is_active, department_id, department:departments(name)", order: "name", columns: [{ key: "code", label: "Code" }, { key: "name", label: "Name" }, { key: "department.name", label: "Department" }, { key: "is_active", label: "Status", badge: true }], fields: [{ name: "department_id", label: "Department", type: "select", required: true, optionsKey: "departments" }, baseFields.code, baseFields.name] },
  { key: "cost_centres", table: "cost_centres", label: "Financial Cost Centres", select: "id, code, name, is_active, department_id, section_id, department:departments(name), section:sections(name)", order: "code", columns: [{ key: "code", label: "Code" }, { key: "name", label: "Name" }, { key: "department.name", label: "Department" }, { key: "section.name", label: "Section" }, { key: "is_active", label: "Status", badge: true }], fields: [{ name: "department_id", label: "Department", type: "select", required: true, optionsKey: "departments" }, { name: "section_id", label: "Section", type: "select", optionsKey: "sections", dependsOn: "department_id" }, baseFields.code, baseFields.name] },
  { key: "expense_categories", table: "expense_categories", label: "Expense Categories", select: "*", order: "name", columns: [{ key: "code", label: "Code" }, { key: "name", label: "Name" }, { key: "is_active", label: "Status", badge: true }], fields: [baseFields.code, baseFields.name, baseFields.description] },
  ...
  { key: "priority_levels", table: "priority_levels", label: "Priority Levels", select: "*", order: "sort_order", columns: [{ key: "code", label: "Priority Code" }, { key: "name", label: "Priority Name" }, { key: "description", label: "Description" }, { key: "sort_order", label: "Rank" }, { key: "is_active", label: "Status", badge: true }], fields: [baseFields.code, baseFields.name, baseFields.description, baseFields.sortOrder] },
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-slate-600">Central master-data administration for organisational, financial and budget lookup values</p>
        </div>
      ...
      <div className="grid gap-2 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700 md:grid-cols-2">
        <p><strong>Organisational Section = where the officer belongs.</strong></p>
        <p><strong>Financial Cost Centre = where expenditure is charged.</strong></p>
        <p><strong>Chart of Accounts = what type of expenditure it is.</strong></p>
        <p><strong>Expense / Posting Code = the valid combined financial coding used for budgeting and transactions.</strong></p>
      </div>
