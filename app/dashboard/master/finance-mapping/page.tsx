...
function statusClass(status: string) {
  ...
  if (status === "INACTIVE_REFERENCE" || status === "SCOPE_MISMATCH") return "bg-red-100 text-red-700"
...
export default function FinanceMappingPage() {
  ...
  const load = useCallback(async () => {
    ...
    setMessage(null)
    try {
      const [mappingRows, ledgerResult, postingResult, accountResult, departmentResult, sectionResult, costCentreResult] = await Promise.all([
        getFinancePostingMappings(),
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div><h2 className="font-semibold text-slate-900">{form.mappingId ? "Edit canonical mapping" : "Create canonical mapping"}</h2><p className="mt-1 text-xs text-slate-500">The technical maker may change master-data mappings only; approved budget lines remain locked.</p></div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field label="Posting Code"><select value={form.expenseCodeRegistryId} onChange={(e) => choosePosting(e.target.value)} className="input"><option value="">Select...</option>{filteredPostings.map((row) => <option key={row.id} value={row.id}>{row.full_expense_code} — {row.description || "No description"}</option>)}</select></Field>
