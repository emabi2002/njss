...
const reportsPagePath = 'app/dashboard/reports/page.tsx'
...
assert.match(scope, /Line Supervisor|sectionId/, 'Section assignment must be part of scope enforcement')

assert.match(page, /authFetch/, 'Management preview must use authenticated fetch rather than direct browser queries')
assert.match(page, /Run Report/, 'Reports page must expose an on-screen Run Report action')
