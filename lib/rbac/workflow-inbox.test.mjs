...
assert.match(route, /\/dashboard\/ff3\/\$\{row\.id\}/, 'Server must issue direct FF3 task links')
assert.match(route, /\/dashboard\/ff4\/\$\{row\.id\}/, 'Server must issue direct FF4 task links')
...
assert.match(page, /href=\{task\.href\}/, 'Inbox must render the server-issued direct transaction link')
assert.match(page, /Open Task/, 'Inbox must provide a direct action to open the transaction')
