...
function requireDestructiveScriptApproval() {
  if (process.env.NJSS_ALLOW_DESTRUCTIVE !== '1') {
    throw new Error('Refusing to run E2E script. Set NJSS_ALLOW_DESTRUCTIVE=1 only for an approved disposable UAT/development environment.')
  }
  const appEnv = (process.env.NEXT_PUBLIC_APP_ENV || '').toLowerCase()
  if (appEnv === 'production' || appEnv === 'prod') {
    throw new Error('Refusing to run E2E script against a production environment.')
  }
}
