...
HELPER = ROOT / "lib/budget-cycle-ui.ts"
TEST = ROOT / "lib/budget-cycle-ui.test.ts"
...
type Subject = {
  findDuplicateBudgetCycle?: (cycles: Cycle[], budgetYear: number, cycleType: string) => Cycle | undefined
  selectBudgetCycle?: (cycles: Cycle[], cycleId: string) => { cycle_id: string; budget_ceiling: string }
}

async function loadSubject(): Promise<Subject> {
  try {
    return (await import("./budget-cycle-ui")) as Subject
  } catch {
    return {}
  }
}
...
HELPER_CONTENT = '''export type BudgetCycleOption = {
  id: string
  budget_year: number
  cycle_type: string
  name: string
...
def run(command: list[str], expect_failure: bool = False) -> None:
    ...
    if expect_failure:
        ...
        print("RED confirmed: the requested behavior is absent before implementation.", flush=True)
        return
    if result.returncode != 0:
...
def main() -> int:
    # RED: write the regression tests before any production code, then confirm failure.
    TEST.write_text(TEST_CONTENT)
    run(["bun", "test", "lib/budget-cycle-ui.test.ts"], expect_failure=True)

    # GREEN: minimal helper logic and approved UI behavior.
...
''',
        "dashboard default-cycle selection",
    )
...
''',
    ...
    run(["bun", "test", "lib/budget-cycle-ui.test.ts"])
    print("GREEN confirmed: regression tests now pass.", flush=True)
    return 0
