...
BEGIN
  ...
  WHERE u.is_active = true
    AND r.name = 'Line Supervisor'
    AND r.is_active = true
    AND public.njss_budget_revision_supervisor_matches(v_division.id, u.id)
...
BEGIN
  ...
  IF NOT public.njss_budget_revision_supervisor_matches(v_division.id, v_supervisor.id) THEN
...
BEGIN
  ...
  IF NOT public.njss_budget_revision_supervisor_matches(v_division.id, v_user_id) THEN
...
SELECT
  ...
  CASE WHEN bd.section_id IS NULL THEN COALESCE(owner_d.id, bd.department_id) ELSE bd.department_id END AS department_id,
...
LEFT JOIN departments owner_d ON owner_d.is_active = true AND owner_d.code = bd.code
...
GROUP BY
  ...
  br.division_id, bd.code, bd.name, bd.department_id, owner_d.id, d.name, bd.section_id,
