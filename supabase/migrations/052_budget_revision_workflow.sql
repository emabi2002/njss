...
-- -----------------------------------------------------------------------------
-- 2. Copied baseline rows retain their posting identity and cannot be deleted.
--    New target rows remain removable while a returned/draft revision is edited.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 3. Internal validation/snapshot helper. Submission snapshots are evidence;
--    approval snapshots are recalculated after authoritative rows are locked.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION njss_validate_budget_revision(p_revision_id UUID, p_snapshot_stage TEXT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
...
BEGIN
  ...
  SELECT * INTO v_submission FROM divisional_budget_submissions
  WHERE id = v_revision.revision_submission_id FOR UPDATE;

  -- New target rows are registered automatically; their baseline is zero.
  ...
  v_bad_line := NULL; v_bad_reason := NULL;
  ...
  IF v_bad_line IS NOT NULL THEN RAISE EXCEPTION 'Revision row % invalid: %', v_bad_line, v_bad_reason; END IF;
  ...
  -- Closed periods and months with actual expenditure are immutable.
  v_bad_line := NULL; v_bad_month := NULL;
  SELECT rl.line_number, rma.month_number
  INTO v_bad_line, v_bad_month
  FROM budget_revision_lines brl
  JOIN divisional_budget_lines rl ON rl.id = brl.revision_budget_line_id
  JOIN budget_monthly_allocations rma ON rma.budget_line_id = rl.id
  JOIN budget_monthly_allocations sma ON sma.budget_line_id = brl.source_budget_line_id AND sma.month_number = rma.month_number
  WHERE brl.budget_revision_id = v_revision.id
    ...
    AND (
      EXISTS (
        ...
        WHERE bp.budget_cycle_id = v_submission.cycle_id
          AND bp.is_active IS DISTINCT FROM false AND bp.is_open = false
          AND (
      ...
      OR EXISTS (
        ...
        WHERE pt.budget_allocation_id = brl.source_budget_allocation_id
          ...
          AND COALESCE(pt.status,'POSTED') <> 'REVERSED' AND COALESCE(pt.amount,0) <> 0
      )
    )
  LIMIT 1;
  IF v_bad_line IS NOT NULL THEN
    RAISE EXCEPTION 'Revision row % month % cannot be changed because the period is closed or has actual expenditure.', v_bad_line, v_bad_month;
  END IF;

  -- Authorised budget changes outside this revision invalidate the baseline.
  v_bad_line := NULL;
  SELECT rl.line_number INTO v_bad_line
  FROM budget_revision_lines brl
  JOIN divisional_budget_lines rl ON rl.id = brl.revision_budget_line_id
  ...
  LIMIT 1;
  IF v_bad_line IS NOT NULL THEN
    RAISE EXCEPTION 'Budget financial position changed for revision row %. Refresh the revision from the current approved budget.', v_bad_line;
  END IF;

  FOR v_line IN
    SELECT brl.id AS revision_line_id, brl.source_budget_allocation_id,
           brl.current_revised_budget, rl.line_number, rl.annual_estimate AS proposed,
           COALESCE((
             SELECT SUM(COALESCE(c.paid_amount,0)) FROM ff3_commitments c
  ...
  LOOP
    IF COALESCE(v_line.proposed,0) + 0.009 < COALESCE(v_line.actual,0) + COALESCE(v_line.outstanding,0) THEN
      RAISE EXCEPTION 'Revision not allowed. Row % has % already spent or committed (protected minimum).',
        v_line.line_number, ROUND(COALESCE(v_line.actual,0)+COALESCE(v_line.outstanding,0),2);
    END IF;
  ...
  SELECT COALESCE(SUM(adjustment_amount),0), COALESCE(SUM(GREATEST(adjustment_amount,0)),0)
  INTO v_net, v_positive
  FROM budget_revision_lines WHERE budget_revision_id = v_revision.id;

  IF v_revision.revision_type IN ('VIREMENT','RECLASSIFICATION') AND ABS(v_net) > 0.009 THEN
    RAISE EXCEPTION 'A virement/reclassification must balance: total decreases must equal total increases.';
  END IF;
  ...
  IF v_revision.revision_type = 'SUPPLEMENTARY' THEN
    IF EXISTS (SELECT 1 FROM budget_revision_lines WHERE budget_revision_id=v_revision.id AND adjustment_amount < -0.009) THEN
      RAISE EXCEPTION 'A supplementary revision cannot contain negative adjustments; use virement/reduction/reclassification as appropriate.';
    END IF;
    IF COALESCE(TRIM(v_revision.authority_reference),'')='' THEN RAISE EXCEPTION 'Supplementary authority reference is required.'; END IF;
    IF v_net <= 0.009 THEN RAISE EXCEPTION 'A supplementary revision must produce a positive net increase.'; END IF;

    SELECT * INTO v_authority FROM funding_authorities fa
    WHERE fa.authority_number=v_revision.authority_reference
      AND fa.financial_year=v_revision.budget_year AND fa.status='APPROVED'
    ORDER BY fa.approved_at DESC NULLS LAST, fa.created_at DESC LIMIT 1;
    ...
    SELECT COALESCE(SUM(fal.allocated_amount),0) INTO v_authority_used
    FROM funding_allocations fal
    WHERE fal.funding_authority_id=v_authority.id AND fal.status='APPROVED';
    IF v_positive > COALESCE(v_authority.approved_amount,0)-v_authority_used+0.009 THEN
    ...
    WHERE brl.budget_revision_id=v_revision.id AND brl.adjustment_amount>0.009
      AND (
    ...
    LIMIT 1;
    IF v_bad_line IS NOT NULL THEN RAISE EXCEPTION 'Supplementary authority restrictions do not permit revision row %.', v_bad_line; END IF;
  END IF;
END;
$$;
...
BEGIN
  ...
  IF NOT (COALESCE(fn_current_user_has_permission('budget.revision.create'),false) OR COALESCE(fn_current_user_has_permission('all'),false)) THEN
    RAISE EXCEPTION 'Permission denied: budget.revision.create is required.';
  END IF;

  SELECT * INTO v_parent FROM divisional_budget_submissions WHERE id=p_parent_submission_id FOR UPDATE;
  ...
  END IF;
  IF COALESCE(TRIM(p_reason),'')='' THEN RAISE EXCEPTION 'Budget revision reason/justification is required.'; END IF;
  IF UPPER(p_revision_type)='SUPPLEMENTARY' AND COALESCE(TRIM(p_authority_reference),'')='' THEN RAISE EXCEPTION 'Supplementary authority reference is required.'; END IF;

  IF EXISTS (SELECT 1 FROM budget_revisions WHERE parent_submission_id=v_parent.id AND status IN ('DRAFT','SUBMITTED','RETURNED','RESUBMITTED','REVIEWED')) THEN
    RAISE EXCEPTION 'An active revision already exists for this approved budget version.';
  END IF;

  SELECT COALESCE(MAX(version),0)+1 INTO v_next_version
  FROM divisional_budget_submissions WHERE cycle_id=v_parent.cycle_id AND division_id=v_parent.division_id;
  ...
  INSERT INTO divisional_budget_submissions (
    ...
    status,validation_status,validation_messages,line_count,total_proposed_budget,total_monthly_allocation,unallocated_variance,is_locked,notes
  ) VALUES (
    v_parent.cycle_id,v_parent.budget_year,v_parent.department_id,v_parent.division_id,v_parent.cost_centre,v_next_version,v_parent.id,NULL,
    v_parent.budget_ceiling,v_parent.ceiling_exception_approved,v_parent.ceiling_exception_reference,v_actor_email,v_actor_email,CURRENT_DATE,
    'DRAFT','PENDING','[]'::jsonb,0,0,0,0,false,'Budget revision created from approved submission '||COALESCE(v_parent.submission_number,v_parent.id::TEXT)
  ...
  FROM budget_revisions WHERE budget_year=v_parent.budget_year AND revision_number~('^REV-'||v_parent.budget_year||'-[0-9]+$');
  ...
  INSERT INTO budget_revisions (
    ...
    v_revision_number,v_parent.id,v_submission_id,v_parent.budget_year,v_parent.division_id,UPPER(p_revision_type),TRIM(p_reason),
    NULLIF(TRIM(p_authority_reference),''),COALESCE(p_effective_date,CURRENT_DATE),'DRAFT',v_user_id,v_actor_email,NULLIF(TRIM(p_supporting_reference),'')
  ) RETURNING id INTO v_revision_id;

  FOR v_source IN SELECT * FROM divisional_budget_lines WHERE submission_id=v_parent.id ORDER BY line_number,id LOOP
    INSERT INTO divisional_budget_lines (
      submission_id,line_number,activity_reference,expense_ledger_id,line_item_description,business_justification,expected_output,
      location_destination_provider,beneficiary_custodian_officer,start_date,end_date,quantity,unit_of_measure,unit_cost,frequency_periods,
      other_costs,priority,funding_source_id,procurement_method,responsible_officer,supporting_reference,comments,
      priority_level_id,procurement_method_id,unit_of_measure_id,responsible_officer_id
    ) VALUES (
      v_submission_id,v_source.line_number,v_source.activity_reference,v_source.expense_ledger_id,v_source.line_item_description,v_source.business_justification,v_source.expected_output,
      v_source.location_destination_provider,v_source.beneficiary_custodian_officer,v_source.start_date,v_source.end_date,v_source.quantity,v_source.unit_of_measure,v_source.unit_cost,v_source.frequency_periods,
      v_source.other_costs,v_source.priority,v_source.funding_source_id,v_source.procurement_method,v_source.responsible_officer,v_source.supporting_reference,v_source.comments,
    ...
    SELECT v_new_line_id,month_number,month_name,amount FROM budget_monthly_allocations WHERE budget_line_id=v_source.id ORDER BY month_number;

    SELECT * INTO v_source_allocation FROM budget_allocations
    WHERE source_budget_line_id=v_source.id AND is_active=true ORDER BY updated_at DESC NULLS LAST,created_at DESC LIMIT 1;
  ...
  PERFORM log_audit_event(
    v_user_id,v_actor_email,COALESCE(v_actor_email,'System'),'BUDGET_REVISION_CREATE','BUDGET_REVISION',v_revision_id,v_revision_number,
  ...
  RETURN jsonb_build_object('revision_id',v_revision_id,'revision_submission_id',v_submission_id,'revision_number',v_revision_number,'version',v_next_version);
END;
$$;
...
-- -----------------------------------------------------------------------------
-- 5. Controlled workflow. Revision APPROVE does not call the initial-budget
--    APPROVE function because that function creates duplicate allocations.
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION njss_transition_budget_revision(p_revision_id UUID,p_action TEXT,p_comments TEXT DEFAULT NULL,p_user_email TEXT DEFAULT NULL)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
    ...
    LOOP
      ...
      IF v_line.source_budget_allocation_id IS NOT NULL THEN
        ...
        SET supplemental_budget=COALESCE(supplemental_budget,0)+CASE WHEN v_revision.revision_type='SUPPLEMENTARY' THEN v_line.adjustment_amount ELSE 0 END,
            ...
            source_budget_submission_id=v_revision.revision_submission_id,source_budget_line_id=v_line.revision_budget_line_id,updated_at=NOW()
