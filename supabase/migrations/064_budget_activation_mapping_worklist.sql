...
-- Exposes approved-budget Finance Code x Cost Centre contexts without guessing
-- accounting classifications or changing approved budget figures.
-- =============================================================================
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    ...
    ORDER BY CASE WHEN m.financial_year = s.budget_year THEN 0 ELSE 1 END,
             m.updated_at DESC,
             m.id
    LIMIT 1
  ) fpm ON true
  LEFT JOIN public.expense_code_registry mapped_posting
    ON mapped_posting.id = fpm.expense_code_registry_id
...
INSERT INTO public.menu_items (
  ...
  'system.finance_mapping_worklist', 'system', 'system.master',
  'Activation Mapping Worklist',
  '/dashboard/master/finance-mapping/worklist',
  'ListChecks', 102,
...
VALUES (
  ...
  to_jsonb('064_budget_activation_mapping_worklist'::TEXT),
